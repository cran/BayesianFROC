...
for (cd in 2:C ){
lower_p[c[cd]] =  inv_psi (   (1-g )* psi(  z_[c[cd-1]] ,m_,v_),m_,v_ ) ;
upper_p[c[cd]] =  inv_psi (   (1-fe)* psi(  z_[c[cd-1]] ,m_,v_),m_,v_ ) ;
...
for(cd in 1 : C-1)   p_[cd] = Phi((z_[cd+1] - m_)/v_)- Phi( (z_[cd] -m_)/v_);
