...
Nowadays, many people are difficult to get job (I am also one of such unemployed person). Such  people have a enough time to spent for reading my poor writing, so, this vignettes are made for such unemployed people. Please Please me! Please employ me (Heart shout)!
...
In this vignette, we provide Bayesian models for FROC analysis. Our work was inspired by a classical 1989 paper of Dev Chakraborty who treated it in the case of Maximal likelihood methods. In classical FROC analysis,  A basic comparison of modality is discussed in terms of ANOVA and in our new methods, we use the posterior mean using hyper-parameter representing AUC. We hope to take up some of these applications in Radiology.
...
##### Comparison of _modalilty_
 ...
 - If Radiographs of patients are grouped by  a treatment group (case) and  another treatment or untreatment (control), then in this context,  modality means __efficacy__ of treatment. That is the AUC of treatment images is greater  than that of untreatment images, then we may say that efficacy of treatment is more significant. 
...
##### $\color{red}{\text{Images, Reader, Gold-standard}}$  
 ...
 - $\color{red}{\text{Images}}$, which contains shadows indicating signals or noises. Also it is called *__case__*. Signal means nodule, lesion, diseased ... etc. __Non-diseased__ image and __diseased__ image are prepared, the former contains no signals and later contains at least one nodule (signal). 
 - $\color{red}{\text{Reader}}$, who  tries to detect lesions from shadows in radiographs. Also it is called *__observer__*
    - reader identifies his suspicious locations from  shadows in each image with his confidence level.
      -  Given an image for reader and researcher ask where do lesions exist?
      -  If  reader thinks "Oh! Definitely lesion!" Then he identify the location with number  5. And if he thinks "What the hell, it is also another lesion,..subtle but,... lesion?", then he marks the low confidence number 2.  
      -  If Reader marked all his suspicious locations, then the researcher give reader a new image. 
      - Be back to step 1.
 ...
 - $\color{red}{\text{Gold Standard of Diagnosis}}$, identifying true lesion locations for all images. One can assign each reader's suspicious location to  *__TP__* (True positive) or *__FP__* (False Positive), whether the location of reader is _correct_ with some  tolerance or not.
...
###### $\color{blue}{\textit{1. Localization with rating}}$
...
- Each yellow triangle indicates reader's suspicious location and the one of them is very near the red circle, thus we may conclude it as hit.
...
Summarizing for the $N_I$-th image, 
 ...
 - Consequently, summing  for $\color{red}{\textit{ all }}$ tables, we ....
...
Consequently, we can say that  $\color{red}{\textit{FROC curve is an expected curve for the FPF and TPF }}$. 
...
where $c$ is the $c^{\text{th}}$ confidence level, and we also call these points the cumulative false positive-cumulative true positive (CFP-CTP) points over $(N_I, N_L)$. 
 If the model is correct, then the FROC curve passes through the expected CFP-CTP points and provides insight into $\color{blue}{\textit{ how the model makes sense as a model. }}$
...
Anyway, in the following, the author provides a lullaby in which the calculation of the area under the AFROC curve is shown.
...
To calculate AUC, we need the notion of convolution.
...
Since $\lambda$ moves from $0$ to $\infty$, the  $x(\lambda) =1-\exp(-\lambda)$ moves $1$ to $0$,
...
Using the definition of $\Phi()$ again, we obtain $$\Phi\biggr(\frac{a}{\sqrt{1+b^2}}\biggr)$$. 

So, my lullaby finished. Many people had gone or be sleeping?,..go to bed? I also want to sleep, here is freedom of expression, no reviewer, ha, Is there someone? help,...
...
The following paper is sufficient to understand my paper (this vignettes) or this package:
...
So, By reading two or three A4 papers, reader would obtains the essence of the modeling for FROC analysis, Cheers! Bye-Bye-kin!
