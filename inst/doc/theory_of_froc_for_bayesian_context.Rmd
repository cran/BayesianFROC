...
#Heuristic and Intuitive explanation
...
In FROC paradigm, reader can answer various choice which leads many hits and many false alarms for each trial.
 ...
 - For each image Reader answer it is disease or not disease.
...
## FROC task (trial)
 ...
 - Suppose there are readers and $N_I$ radiographs and researcher with the *gold* *standard* ${}^\dagger$
 - We assume that Each radiographs contains shadows and researcher knows which shadows are targets that reader should find. Of course, readers do not know which shadow is actual true lesions (target, nodule).
 - Each radiograph contains shadows which is lesion (nodule, target) or healthy one (noise). 
 ...
 - After readers answered all his suspicious locations in radiographs with his  confidence levels, researcher check and count the ${}^{\dagger \dagger }$ hits $H_c$ and ${}^{\dagger \dagger \dagger}$ false alarms $F_c$ for all answer of reader for each confidence level $c$.
...
1 = questionable       |  $H_{1}$ |  $F_{1}$ 
 ...
 - "Model" means the probability with which the above data arise.
 ...
 - That is, the thresholds and the  mean and standard deviation of the signal distribution.
...
- if $Y_l>t$ for some positive real number $t$, then reader generate the $l$-th lesion is diseased 
and
...
These variables decide whether reader think it is positive or not according to its value is greater than certain threshold. Thus it is also called *decision* variables. 
 ...
 - Latent Gaussian = latent bi-normal = decision variable = ... etc.
...
To combine $1-x(t)$ to the false alarm rate, recall that $x(t):=\mathbb{P}[X_i>t]$, so  $1-x(t):=\mathbb{P}[X_i<t]$.
...
$$z_2 - z_1 \sim \text{Uniform}(0,\infty) \\ 
  ...
  $$
  where Uniform$(0,\infty)$ means improper prior whose support is the interval $(0,\infty)$.
