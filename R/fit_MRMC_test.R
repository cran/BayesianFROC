...
# devtools::document();help("fit_MRMC_test") # Confirm reflection



#' @export fit_MRMC_test

fit_MRMC_test<- function(
                      dataList,
                      DrawCurve = FALSE,
...
if(Null.Hypothesis==FALSE){
  if(PreciseLogLikelihood == FALSE  ){
    # scr <- "Model_Hiera.stan";
  scr <- system.file("extdata", "Model_Hiera.stan", package="BayesianFROC")

  }else{
    if(PreciseLogLikelihood == TRUE  ){
      # scr <- "Model_Hiera_TargetFormulation.stan";
  scr <-  system.file("extdata", "Model_Hiera_TargetFormulation.stan", package="BayesianFROC")
...
if(summary ==TRUE){
  ...
  #   dig=dig
  # )
  summarize_MRMC(fit.new.class,dig=dig)
}#if
