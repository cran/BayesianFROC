...
#'and single modality data \code{dataList}. FPF is \emph{\strong{ per lesion}}
...
#'@examples
#' \donttest{
#'#First, we prepare the example data.
#'
...
# devtools::document();help("fit_srsc_per_lesion") # Confirm reflection
  ...
  if(DrawCurve == TRUE  ){
    ...
    if(DrawAFROCcurve==TRUE){
      ...
      suppressWarnings(graphics::par(new=T)); plot(x,y,
                                      ...
                                      cex= 0.1 ,
                                      xlim = c(0,upper_x ),ylim = c(0,upper_y),
                                      xlab = paste('mean of cumulative false positives per lesion (', NL," = number of lesions)"  ),
                                      ylab = paste('mean of cumulative hit per lesion (', NL," = number of lesions)"  ),
                                      ,main =title
      );
