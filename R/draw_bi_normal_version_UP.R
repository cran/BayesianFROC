...
draw_bi_normal_version_UP <- function( StanS4class,     dark_theme =TRUE, dig =3,mesh=1000, new.imaging.device = TRUE,
  ...
  if(both.hit.and.false.rate==TRUE) {
    ...
    graphics::curve( d_log_Phi(x), ########################### 2019 July
                     min(m-3*v,0-1),max(m+3*v,0+1) ,col = noise.col, ylim=c(0, stats::dnorm(m,m,v))
  ...
  if (false.alarm.rate==TRUE){
    ...
    graphics::curve(d_log_Phi(x)########################### 2019 July
                    ...
                    ,min(m-3*v,0-1),max(m+3*v,0+1) ,col = noise.col
                    ,xlab = "Latent Logarithmic Gaussian, Signal distribution  and thresholds",
                    ylab = "Probability Density",
